...
public class DriverManage {
    ...
    public int repAuto(Statement s, String answer, int iddmd) {
        int idrep = 0;
        try {
            int statu = s.executeUpdate("INSERT INTO auto (reponse,iddemde) VALUES ('" + answer + "','" + iddmd + "')");
            System.out.println("!!! REPONSE RECU !!!");
            ResultSet result = s.executeQuery("SELECT id FROM auto WHERE iddemde='" + iddmd + "'");
            result.next();
            idrep = result.getInt("id");
